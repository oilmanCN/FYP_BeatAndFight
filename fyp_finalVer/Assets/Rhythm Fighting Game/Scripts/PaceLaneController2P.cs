...
namespace SonicBloom.Koreo.Demos
{
    ...
    public class PaceLaneController2P : MonoBehaviour
    {
        #region Fields
        ...
        Queue<PaceNoteObject2P> trackedNotes = new Queue<PaceNoteObject2P>();

        // A reference to the Rythm Game Controller.  Provides access to the NoteObject pool and other parameters.
        PaceController2P paceController2P;

        public event Action<String, String> moveCommand;

        public static PaceLaneController2P Instance { get; private set; }
        ...
        public void Initialize(PaceController2P controller)
        {
            paceController2P = controller;
        }
        ...
        void Start()
        {
            ...
            despawnX = Camera.main.ViewportToWorldPoint(new Vector3(0f, 0f, cameraOffsetZ)).x + 13.3f; // original + 12.5f
        ...
        void Update()
        {
            ...
            if (Input.GetKeyDown(keyboardButtonPositive))
            {
                if (CheckNoteHit())
                {
                    P2Movement.Instance.AvatarMove(keyboardButtonPositive);
                }
                SetScalePress();
            }
            else if (Input.GetKeyDown(keyboardButtonNegetive))
            {
                if (CheckNoteHit())
                {
                    P2Movement.Instance.AvatarMove(keyboardButtonNegetive);
                }
                SetScalePress();
        ...
        int GetSpawnSampleOffset()
        {
            ...
            double spawnSecsToTarget = (double)spawnDistToTarget / (double)paceController2P.noteSpeed;

            // Figure out the samples to the target.
            return (int)(spawnSecsToTarget * paceController2P.SampleRate);
        }
        ...
        public bool CheckNoteHit()
        {
            ...
            {
                PaceNoteObject2P hitNote = trackedNotes.Dequeue();
        ...
        void CheckSpawnNext()
        {
            ...
            int currentTime = paceController2P.DelayedSampleTime;
                   ...
                   laneEvents[pendingEventIdx].StartSample < currentTime + samplesToTarget)
            {
                ...
                PaceNoteObject2P newObj = paceController2P.GetFreshNoteObject();
                newObj.Initialize(evt, color, this, paceController2P);
