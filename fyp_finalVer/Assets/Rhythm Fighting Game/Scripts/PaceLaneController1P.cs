...
{
    [AddComponentMenu("Koreographer/Demos/Rhythm Game/Pace Lane Controller 1P")]
    public class PaceLaneController1P : MonoBehaviour
    {
        #region Fields
        ...
        public KeyCode keyboardButtonNegetive;
        ...
        Queue<PaceNoteObject1P> trackedNotes = new Queue<PaceNoteObject1P>();
        ...
        public event Action<String, String> moveCommand;

        public static PaceLaneController1P Instance { get; private set; }
        ...
        public void Initialize(PaceController1P controller)
        {
            paceController1P = controller;
        }
        ...
        void Start()
        {
            ...
            despawnX = Camera.main.ViewportToWorldPoint(new Vector3(0f, 0f, cameraOffsetZ)).x + 12.7f;  // original + 12.5f  //modified +14f
            ...
            p1move = FindObjectOfType<P1Movement>();
        ...
        void Update()
        {
            ...
            if (Input.GetKeyDown(keyboardButtonPositive))
            {
                if (CheckNoteHit()) {
                    //print(p1move);
                    //p1move.AvatarMove(keyboardButtonPositive);
                    P1Movement.Instance.AvatarMove(keyboardButtonPositive);
                }
                SetScalePress();
            }
            else if (Input.GetKeyDown(keyboardButtonNegetive)) 
            {
                if (CheckNoteHit()) 
                {
                    //p1move.AvatarMove(keyboardButtonNegetive);
                    P1Movement.Instance.AvatarMove(keyboardButtonNegetive);
                }
                SetScalePress();

            }
            else if (Input.GetKey(keyboardButtonPositive) || Input.GetKey(keyboardButtonNegetive))
            {
                SetScaleHold();
            }
            else if (Input.GetKeyUp(keyboardButtonPositive) || Input.GetKeyUp(keyboardButtonNegetive))
            {
                SetScaleDefault();
            }
        }
        ...
        int GetSpawnSampleOffset()
        {
            ...
            double spawnSecsToTarget = (double)spawnDistToTarget / (double)paceController1P.noteSpeed;

            // Figure out the samples to the target.
            return (int)(spawnSecsToTarget * paceController1P.SampleRate);
        }
        ...
        public bool CheckNoteHit()
        {
            ...
            {
                PaceNoteObject1P hitNote = trackedNotes.Dequeue();
        ...
        void CheckSpawnNext()
        {
            ...
            int currentTime = paceController1P.DelayedSampleTime;
                   ...
                   laneEvents[pendingEventIdx].StartSample < currentTime + samplesToTarget)
            {
                ...
                PaceNoteObject1P paceObj = paceController1P.GetFreshNoteObject();
                paceObj.Initialize(evt, color, this, paceController1P);

                trackedNotes.Enqueue(paceObj);
        ...
        public void commandMove(String direction)
        {
            moveCommand?.Invoke("Player1", direction);
        }
