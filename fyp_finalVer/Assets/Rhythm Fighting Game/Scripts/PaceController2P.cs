...
{
    [AddComponentMenu("Koreographer/Demos/Rhythm Game/Pace Controller 2P")]
    public class PaceController2P : MonoBehaviour
    {
        #region Fields
        ...
        [Range(8f, 150f)]
        public float hitWindowRangeInMS = 60f;
        ...
        public PaceNoteObject2P noteObjectArchetype;

        [Tooltip("The list of Lane Controller objects that represent a lane for an event to travel down.")]
        public List<PaceLaneController2P> noteLanes = new List<PaceLaneController2P>();
        ...
        Koreography paceKoreo;
        SimpleMusicPlayer playingMusic;
        ...
        Stack<PaceNoteObject2P> noteObjectPool = new Stack<PaceNoteObject2P>();

        public static PaceController2P Instance { get; private set; }
        ...
        public int DelayedSampleTime
        {
            get
            {
                ...
                return paceKoreo.GetLatestSampleTime() - (int)(audioCom.pitch * leadInTimeLeft * SampleRate);
            }
        }
        ...
        void Start()
        {
            ...
            eventID = SelectedSongController.Instance.buttonPace;
            paceKoreo = Koreographer.Instance.GetKoreographyAtIndex(songIndex);
            eventDelay = OptionController.Instance.timeDelay;
            //playingMusic = FindObjectOfType<SimpleMusicPlayer>();
            //playingMusic.LoadSong(paceKoreo);
            // Grab all the events out of the Koreography.
            KoreographyTrack paceTrack = paceKoreo.GetTrackByID(eventID);
            //print("2P:" + paceKoreo);
            //print("2P:" + paceTrack);
            List<KoreographyEvent> rawEvents = paceTrack.GetAllEvents();
            ...
            for (int i = 0; i < rawEvents.Count; ++i)
            {
                ...
                {
                    PaceLaneController2P lane = noteLanes[j];
        ...
        void InitializeLeadIn()
        {
            ...
            if (leadInTime > 0f)
            {
                ...
                timeLeftToPlay = leadInTime - eventDelay;
            }
            else
            {
        ...
        {
            hitWindowRangeInSamples = (int)(0.001f * hitWindowRangeInMS * SampleRate);  //change the judgment area   
        }

        // Retrieves a frehsly activated Note Object from the pool.
        public PaceNoteObject2P GetFreshNoteObject()
        {
            PaceNoteObject2P retObj;
            ...
            {
                retObj = GameObject.Instantiate<PaceNoteObject2P>(noteObjectArchetype);
            }
        ...
        public void ReturnNoteObjectToPool(PaceNoteObject2P obj)
        {
            if (obj != null)
            {
                obj.enabled = false;
        ...
        public void Restart()
        {
            ...
            Koreographer.Instance.FlushDelayQueue(paceKoreo);
